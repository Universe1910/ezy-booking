...
// GetEzyCustomerByPhone 根据id获取EzyCustomer记录
// Author [piexlmax](https://github.com/piexlmax)
func (ezyCustomerService *EzyCustomerService) GetEzyCustomerByPhone(phone string) (ezyCustomer Booking.EzyCustomer, err error) {
	err = global.GVA_DB.Where("phone = ?", phone).First(&ezyCustomer).Error
	return
}
